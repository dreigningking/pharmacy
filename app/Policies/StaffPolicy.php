...
class StaffPolicy
{
    ...
    public function view(User $user, PharmacyUser $pharmacyUser)
    {
        ...
        $permitted_roles = Permission::where('name','user')->first()->roles->where("pivot.view",1)->pluck('id')->toArray();
        $checker = false;
    ...
    public function create(User $user)
    {
        ...
        $permitted_roles = Permission::where('name','user')->first()->roles->where("pivot.new",1)->pluck('id')->toArray();
        $checker = false;
        foreach($permitted_roles as $permitted){
            if(in_array($permitted,$roles)){
                $checker = true;
    ...
    {
        $roles = $user->pharmacies->pluck('pivot.role_id')->toArray();
        $permitted_roles = Permission::where('name','user')->first()->roles->where("pivot.list",1)->pluck('id')->toArray();
        $checker = false;
        foreach($permitted_roles as $permitted){
            if(in_array($permitted,$roles)){
                $checker = true;
    ...
    {
        $roles = $user->pharmacies->where('id',$pharmacyUser->pharmacy_id)->pluck('pivot.role_id')->toArray();
        $permitted_roles = Permission::where('name','user')->first()->roles->where("pivot.edit",1)->pluck('id')->toArray();
        $checker = false;
        foreach($permitted_roles as $permitted){
            if(in_array($permitted,$roles)){
                $checker = true;
    ...
    {
        $roles = $user->pharmacies->where('id',$pharmacyUser->pharmacy_id)->pluck('pivot.role_id')->toArray();
        $permitted_roles = Permission::where('name','user')->first()->roles->where("pivot.remove",1)->pluck('id')->toArray();
        $checker = false;
        foreach($permitted_roles as $permitted){
            if(in_array($permitted,$roles)){
                $checker = true;
